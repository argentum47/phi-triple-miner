#!/bin/bash
# =======================================================
# Triple-miner launch script v1.3
# -------------------------------------------------------
...
# Estimated system draw: 1,110 watts
...
# --------------------------
XMR_HOST="stratum+tcp://stratum.cudopool.com"
XMR_PORT=30010
...
# 4x GTX 1050ti @ ~15 MH/s each = ~60 MH/s combined @ ~240 watts
# --------------------------
...
log "========== phi-triple-miner is starting =========="
...
# lukMiner removed in v1.2 (no viable CryptoNight Haven targets)
# BOINC does not use huge pages
# Dual E5-2698 v4 (2 NUMA nodes): ~1,600 pages per node = ~3,200 pages
# 3,700 total with buffer. Significantly reduced from v1.1 (was 13,000)
# --------------------------
...
# BOINC clients run on native Linux on-card; if MPSS is not running
...
# --------------------------
start_xmrig() {
    log "Starting xmrig on 2x E5-2698 v4 — XMR via CudoMiner..."
    while true; do
...
# --------------------------
# Watchdog: T-Rex (4x GTX 1050ti — ETC Etchash @ F2Pool)
# --intensity 20: safe default for 1050ti 4GB cards
# --api-port 4067: exposes local API for XMRigCC monitoring
# --------------------------
start_trex() {
    log "Starting T-Rex on 4x GTX 1050ti — ETC via F2Pool..."
    while true; do
